...
import { assert, getTestCount, getWorkspaceUri, log, runAllTests, Uri, suiteSetupCommon } from '../testCommon'
...
suite('proj5Suite', () => {
	...
	test('proj5.1 - test count', async () => {
		...
		const resultsJson = Uri.joinPath(getWorkspaceUri(), 'ablunit', 'results.json')
		const testCount = await getTestCount(resultsJson)
		assert.assert(testCount > 100)
	})
