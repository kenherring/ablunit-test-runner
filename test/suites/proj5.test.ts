...
suite('proj5 - Extension Test Suite', () => {
	...
	test('proj5.3 - TestClass - login/test2.cls - ablunit.explorer.classlabel=class-type-name (default)', async () => {
		const [ lines, ] = getAnnotationLines(await getContentFromFilesystem('test/login/test2.cls'), '@test')
	...
	test('proj5.5 - TestClass - login/test5.cls - test count', async () => {
		const [ lines, ] = getAnnotationLines(await getContentFromFilesystem('test/login/test5.cls'), '@test')
	...
	test('proj5.6 - TestClass - login/test5.cls - test count', async () => {
		const [ lines, ] = getAnnotationLines(await getContentFromFilesystem('test/login/test7.cls'), '@test')
	...
	test('proj5.8 - TestProcedure - test/proc2/test7.p - test count', async () => {
		const [ lines, ] = getAnnotationLines(await getContentFromFilesystem('test/proc2/proc7.p'), '@test')
		const classRet = parseTestProgram(lines, 'filename')
