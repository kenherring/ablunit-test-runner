...
test('proj7B.2 - cancel test run while adding tests', async () => {
	...
	runAllTests(true, false)
		.then(() => {
			assert.fail('runAllTests completed without throwing CancellationError')
		}, (e: unknown) => {
			log.info('runAllTests got error: ' + e)
			if (e instanceof CancellationError) {
				log.info('testing.runAllTests threw CancellationError as expected ' + runTestTime.toString())
				return
			}
			assert.fail('testing.runAllTests threw unexpected error. Expected e.name="Canceled" e=' + e)
		})
	...
	// 	} else {
	// 		assert.fail('runAllTests threw unexpected error. Expected e.name="Canceled" e=' + e)
	// 	}
	// })
})
