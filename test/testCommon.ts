...
import { IRunProfile as IRunProfileGlobal } from '../src/parse/config/RunProfile'
...
// ablunit-test-runner extension objects
...
export async function setRuntimes (runtimes?: IRuntime[]): Promise<number> {
	if (!enableExtensions()) {
...
export async function waitForTestRunStatus (waitForStatus: RunStatus) {
	...
	log.info('found test run status    = \'' + currentStatus.toString() + '\'')
	// @ts-expect-error 123
	log.info('found test run status    = \'' + currentStatus['key'] + '\'')
	log.info('comparing to run status  = \'' + waitForStatus + '\'')
	log.info('comparing to run status  = \'' + waitForStatus.toString() + '\'')
	// @ts-expect-error 123
	log.info('comparing to run status  = \'' + waitForStatus['key'] + '\'')
