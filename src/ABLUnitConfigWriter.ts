...
import { createDir, deleteFile } from 'ABLUnitCommon'
...
export class ABLUnitConfig  {
	...
	writeFile (uri: Uri, data: Uint8Array) {
		createDir(uri.with({ path: uri.path.split('/').slice(0, -1).join('/') }))
	...
	createProgressIni (propath: string, dlc: IDlc) {
		...
		// TODO - only write if the existing file has the generated by comment
	...
	createAblunitJson (_uri: Uri, cfg: CoreOptions, testQueue: ITestObj[]) {
		...
		createDir(this.ablunitConfig.optionsUri.locationUri)
		deleteFile(this.ablunitConfig.optionsUri.filenameUri)
		deleteFile(this.ablunitConfig.optionsUri.jsonUri)
	...
	createProfileOptions (uri: Uri, profOpts: ProfilerOptions) {
		...
		if (this.ablunitConfig.profListingsUri) {
			...
			createDir(this.ablunitConfig.profListingsUri)
		}
		...
		deleteFile(this.ablunitConfig.profFilenameUri)
