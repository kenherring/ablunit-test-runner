...
import { assert, deleteTestFiles, getWorkspaceUri, log, runAllTests, updateConfig, waitForExtensionActive } from '../testCommon'
// import { afterEach, before, beforeEach } from 'mocha'
...
suite(projName + ' - Extension Test Suite', () => {
	...
	test(projName + '.1 - output files exist - 1', async () => {
		...
		log.info("wsl=" + process.env['WSL_DISTRO_NAME'])
		log.info("ablunitJson=" + ablunitJson.fsPath)
		log.info("resultsXml=" + resultsXml.fsPath)
		log.info("resultsJson=" + resultsJson.fsPath)
